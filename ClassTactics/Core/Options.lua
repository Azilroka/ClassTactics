...
CT.Options.args.general.args.discordLinks.args.raidbots = ACH:Input('Raidbots', nil, nil, nil, 'full', function() return 'https://discord.gg/86EF64Q' end)
	...
	for index = 1, 12 do
		...
		if tag == classTag then
			for specGroup = 1, GetNumSpecializationsForClassID(classID) do
				...
				tinsert(specTable, specName)
			end

			return name, specTable
		end
	end
end
...
function CT:GetTalentBuildOptions(classTag, specIndex)
	...
	for talentName in next, CT.Data[classTag][specIndex].Talents do
...
function CT:BuildOptions()
	...
	-- Class Section
	for _, classTag in next, CLASS_SORT_ORDER do
		local className, specInfo = CT:GetClassInfoByToken(classTag)
		...
		for specGroup, specName in next, specInfo do
			...
			specOption.args.ActionBars.args.General = ACH:MultiSelect('General', nil, 0, { createMissingMacros = 'Create Missing Macros', preferCharacterMacros = 'Prefer Character Macros'}, nil, nil, function(_, key) return key end, function(_, key, value) CT.db[key] = value end)

			specOption.args.ActionBars.args.buttons = ACH:Group(' ', nil, 1)
			specOption.args.ActionBars.args.buttons.inline = true
			specOption.args.ActionBars.args.buttons.args.LoadActionBarSet = ACH:Execute('Load ActionBar Set', nil, 2, function() CT:LoadActionSet(CT.OptionsData[classTag][specGroup].SelectedActionBarSet) end, nil, nil, nil, nil, nil, function() return classTag ~= CT.MyClass or specGroup ~= GetSpecialization() or not CT.OptionsData[classTag][specGroup].SelectedActionBarSet or CT.OptionsData[classTag][specGroup].SelectedActionBarSet == '' or CT.OptionsData[classTag][specGroup].SelectedActionBarSet == 'NONE' end)
			specOption.args.ActionBars.args.buttons.args.SaveActionBarSet = ACH:Execute('Save ActionBar Set', nil, 3, function() CT:SetupActionBarPopup(CT.OptionsData[classTag][specGroup].SelectedActionBarSet) end, nil, nil, nil, nil, nil, function() return classTag ~= CT.MyClass or specGroup ~= GetSpecialization() end)
			specOption.args.ActionBars.args.buttons.args.DeleteActionBarSet = ACH:Execute('Delete ActionBar Set', nil, 4, function() CT:DeleteActionBarSet(classTag, specGroup, CT.OptionsData[classTag][specGroup].SelectedActionBarSet) CT.OptionsData[classTag][specGroup].SelectedActionBarSet = nil end, nil, nil, nil, nil, nil, function() return not CT.OptionsData[classTag][specGroup].SelectedActionBarSet or CT.OptionsData[classTag][specGroup].SelectedActionBarSet == '' or CT.OptionsData[classTag][specGroup].SelectedActionBarSet == 'NONE' end)

			specOption.args.ActionBars.args.ActionBarSets = ACH:MultiSelect('ActionBar Sets', nil, -2, function() return CT:GetActionBarSets(classTag, specGroup) end, nil, nil, function(_, key) return CT.OptionsData[classTag][specGroup].SelectedActionBarSet == key end, function(_, key) CT.OptionsData[classTag][specGroup].SelectedActionBarSet = key end)
